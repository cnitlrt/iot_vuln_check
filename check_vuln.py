    ...
    for block in func.low_level_il:
        for instr in block:
            ...
            if instr.operation == binaryninja.LowLevelILOperation.LLIL_CALL:
                # 获取被调用的函数
                called_func = bv.get_function_at(instr.operands[0].value.value)
                # 检查函数名是否为'sprintf'
                if called_func == None:
                    continue
                if called_func.name == symbol:
                    x.append(instr.address)
    return x
        ...
        for call_sprintf_addr in call_sprintf_addrs:
            ...
            while True:
                ...
                if fmt_value[idx:].startswith("%s"):
                    arg = func.get_parameter_at(call_sprintf_addr,None,arg_idx)
                    if not is_constant(arg) and func.get_parameter_at(call_sprintf_addr,None,0).value == cmd.value:
                        x = ((symbol,func.name,ref.address,Success.COMMANDINJECT))
                        if x not in ret:
                            ret.append(x)
                        break
